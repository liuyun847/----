...
class TestParseExpression:
    ...
    def test_parse_expression_time_units_hour(self):
        ...
        assert parse_expression("15/hour") == pytest.approx(0.004166666666666667)
    
    def test_parse_expression_complex_with_time(self):
        """测试复合表达式和时间单位"""
        assert parse_expression("5*2/3/min") == pytest.approx(0.05555555555555555)
        assert parse_expression("2.5*3.14/h") == pytest.approx(0.0021805555555555556)
        assert parse_expression("(10+5)*2/60/min") == pytest.approx(0.008333333333333333)
...
class TestConvertTimeUnit:
    ...
    def test_convert_hour(self):
        """测试小时单位转换"""
        assert convert_time_unit(15, "h") == pytest.approx(0.004166666666666667)
        assert convert_time_unit(15, "hour") == pytest.approx(0.004166666666666667)
