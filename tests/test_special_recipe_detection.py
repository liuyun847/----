...
class TestDoublingRecipe:
    ...
    def test_calculate_net_output_doubling(self, analyzer, doubling_recipe):
        """测试计算倍增配方净产出"""
        net_output = analyzer.calculate_net_output_for_item(doubling_recipe, "a")
...
class TestCatalystRecipe:
    ...
    def test_calculate_net_output_catalyst(self, analyzer, catalyst_recipe):
        """测试计算催化剂配方净产出"""
        net_output_a = analyzer.calculate_net_output_for_item(catalyst_recipe, "a")
        net_output_b = analyzer.calculate_net_output_for_item(catalyst_recipe, "b")
        net_output_c = analyzer.calculate_net_output_for_item(catalyst_recipe, "c")
...
class TestInvalidRecipe:
    ...
    def test_calculate_net_output_zero(self, analyzer, invalid_zero_output_recipe):
        """测试计算零产出配方净产出"""
        net_output = analyzer.calculate_net_output_for_item(invalid_zero_output_recipe, "a")
        assert net_output == 0.0  # 1.0 - 1.0 = 0.0
    
    def test_calculate_net_output_negative(self, analyzer, invalid_negative_output_recipe):
        """测试计算负产出配方净产出"""
        net_output = analyzer.calculate_net_output_for_item(invalid_negative_output_recipe, "a")
        assert net_output == -1.0  # 1.0 - 2.0 = -1.0
    
    def test_is_valid_production_invalid(self, analyzer, invalid_zero_output_recipe, invalid_negative_output_recipe):
        """测试无效配方有效性验证"""
        assert analyzer.is_valid_production_recipe(invalid_zero_output_recipe) is False
        assert analyzer.is_valid_production_recipe(invalid_negative_output_recipe) is False
